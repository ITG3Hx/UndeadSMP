...
const FIELDS = [
  'tickets-enabled', 'tickets-categoryId', 'tickets-staffRoleIds', 'tickets-leadModeratorRoleIds',
  'tickets-transcriptChannelId', 'tickets-panelTitle', 'tickets-panelText', 'tickets-buttonLabel',
  'tickets-openMessage',
...
function fillSelect(select, items, current, emptyLabel) {
  ...
  const currentIds = isMulti ? current || [] : [current];
  select.innerHTML = '';
  ...
  for (const item of items) {
    ...
    option.selected = currentIds.includes(item.id);
    select.appendChild(option);
  }

  // IDs aus der Config, die es auf dem Server nicht mehr gibt (geloeschter Channel/Rolle):
  // trotzdem anzeigen, damit sie nicht beim naechsten Speichern still verschwinden
  for (const id of currentIds) {
    if (!id || items.some((item) => item.id === id)) continue;
    const option = document.createElement('option');
    option.value = id;
    option.textContent = `${id} (nicht gefunden)`;
    option.selected = true;
    select.appendChild(option);
  }

  if (!isMulti) select.value = current || '';
}
...
async function load() {
  ...
  fillSelect($('tickets-staffRoleIds'), guild.roles, config.tickets.staffRoleIds, 'Keine Rolle');
  fillSelect($('tickets-leadModeratorRoleIds'), guild.roles, config.tickets.leadModeratorRoleIds, 'Keine Rolle');
...
async function save() {
  ...
  for (const field of FIELDS) {
    const [section, key] = field.split('-');
    const element = $(field);
    if (element.type === 'checkbox') patch[section][key] = element.checked;
    else if (element.multiple) patch[section][key] = [...element.selectedOptions].map((option) => option.value);
    else patch[section][key] = element.value;
  }
