...
async function openTicket(interaction) {
  ...
  if (tickets.staffRoleId && guild.roles.cache.has(tickets.staffRoleId)) {
    overwrites.push({ id: tickets.staffRoleId, allow: [...MEMBER_PERMS, PermissionFlagsBits.ManageMessages] });
  }
  ...
  await channel.send({
    content: tickets.staffRoleId ? `<@&${tickets.staffRoleId}>` : null,
    embeds: [embed],
    components: [row]
  });
...
export async function closeTicket(interaction) {
  ...
  const isStaff =
    (tickets.staffRoleId && interaction.member.roles.cache.has(tickets.staffRoleId)) ||
